...
int vNum = 3;						# Number of vertical boxes
int hNum	= 4;					# Nomber of horizontal boxes
...
array<string> picPairsForMat[6][3];
#Read Items Function
####################################
	...
	loop  int i = 1 until i > picsCat_U.count()
		begin
			...
			if(i<=3)then
				...
				picPairsForMat[i][1] =  picsCat_U[i];
				picPairsForMat[i][2] =  picsCat_O[i-3];
				
				if(i-3<3)then
					picPairsForMat[i][3] =  picsCat_O[i-3+1];
				else
...
id = id +"-PairList-PosMatrix-TestRun.txt";
...
#### End Randomization #########	
	term.print_line(string(deltatime));
...
# go through picPairs
	...
	begin
				...
				picStim.set_part( 13,presPic );
				array<int> out[] = getItemPos(picMatrix,picPairs[i][1],ZEILEN,SPALTEN);

				picStim.set_part_x(13,(spacing * (out[1] - (SPALTEN+1)/2))-spacing/2);
				#picStim.set_part_x(41,150* (8 - (SPALTEN1)/2));
				picStim.set_part_y(13,-spacing * (out[2] - (ZEILEN+1)/2));
				...
				#
				#		Fix.set_part_x(1,(150 * (out[1] - (SPALTEN+1)/2))-75);
				#		Fix.set_part_y(1,-150 * (out[2] - (ZEILEN+1)/2));
				#	
